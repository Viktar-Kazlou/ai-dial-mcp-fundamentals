...
@mcp.tool(description="Get full user information by user ID.")
async def get_user_by_id(id: int) -> str:
    return await user_client.get_user(id)


@mcp.tool(description="Delete user by user ID.")
async def delete_user(id: int) -> str:
    return await user_client.delete_user(id)


@mcp.tool(description="Search users by optional filters: name, surname, email, gender.")
async def search_user(
    ...
    search_request = UserSearchRequest(name=name, surname=surname, email=email, gender=gender)
...
@mcp.tool(description="Add a new user profile.")
async def add_user(user: UserCreate) -> str:
    return await user_client.add_user(user)


@mcp.tool(description="Update an existing user profile by user ID.")
...
@mcp.resource(
    ...
    mime_type="image/png",
    description="Flow diagram with User Service swagger endpoints."
)
...
@mcp.prompt(name="search_user_guidance", description="Guidance for effective user search queries.")
async def search_user_guidance() -> str:
    return SEARCH_PROMPT


@mcp.prompt(name="create_user_profile_guidance", description="Guidance for creating realistic user profiles.")
